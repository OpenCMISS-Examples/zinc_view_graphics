...
You need to install Python, the OpenCMISS-Zinc library, and PyZinc (Python bindings to Zinc) as described in earlier tutorials. For the user interface you also require Qt, PySide2 (Qt for Python) and the `opencmiss.zincwidgets <https://github.com/OpenCMISS-Bindings/opencmiss.zincwidgets>`_ which supply reusable widgets for showing graphics and building user interfaces with Zinc. Instructions for getting these are found elsewhere on the opencmiss.org documentation.
